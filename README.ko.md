...
하나의 외부 라이브러리를 여러 로컬 하네스에서 공유할 수 있습니다. 파일 해시가 바뀌면 캐시를 무효화하고, 지속 실행되는 MCP 프로세스는 HTTP 연결과 짧은 메모리 캐시를 재사용합니다. 별도 CLI 실행이나 프롬프트마다 새로 뜨는 Claude 훅 프로세스 사이에는 캐시가 공유되지 않습니다.
