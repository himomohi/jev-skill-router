...
The code preflights conservative verification byte bounds using character-width histograms computed once per loaded body. Only shortlisted candidates generate task-specific excerpts after ranking. Fixed verification slots preserve the reserved call bound. Extremely escape-heavy or mixed-width text with tight limits may be rejected before any API request; reducing `excerpt_chars` can help. Actual HTTP sends, including retries, consume a shared runtime allowance. Ranking batches run concurrently; verification starts only after ranking succeeds. Results are combined in catalog order. Asynchronous network requests and queued work are cancelled on failure or deadline, and cancellation is awaited before returning. Already accepted provider requests may still be billed; cancellation does not undo server work. Filesystem scans and local CPU work are checked at phase boundaries and cannot be preempted by the network deadline. Constructor-time initial catalog loading is outside the route timer.
