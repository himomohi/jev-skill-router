...
The recorded v0.2.1 Linux/Python 3.12 run used 200 synthetic local files and five alternating repetitions per mode: median **147.313 ms** for full refresh and **21.975 ms** for incremental refresh; unchanged file reloads fell from **200 to 0**. [Raw samples](runtime-benchmark.json). This measures real local traversal/read/parse/hash work, not an old-release end-to-end comparison. Directory checks remain; OS caches and filesystem behavior affect results. Windows can reuse files with native NTFS/ReFS change metadata and otherwise uses full reads; no Windows timing measurement is included. No Jev speed or cost claim follows from this measurement.

The v0.3.0 illustrative envelope includes revision-aware read arguments, a file digest and physical-request/elapsed-time fields. The same selected read envelope (including its digest) is counted on both sides. Its 200-skill result is **58,088 → 4,845 bytes (91.66%)**. The v0.2.1 sample was 58,004 → 4,463 bytes (92.31%); v0.1.0 used 4,403 routed bytes (92.41%). The additional safety metadata increases overhead. Historical media retains the earlier illustration. Five skills now produce **24.88% more** modeled context, so small inventories should not assume a context benefit.


The v0.4.0 envelope additionally includes explicit retrieval scope. The current 200-skill figure is **58,088 → 4,940 bytes (91.50%)**; five skills increase the modeled payload by **27.28%**. The prior paragraph records the historical v0.3.0 sample. See [request scaling](SCALING.md) for provider request bounds and [comparative evaluation](COMPARATIVE_EVALUATION.md) for authored routing cases. These are separate measurements with separate denominators.
