...
| Cold latency | Fresh wall-clock duration with the decision cache cleared before every case, including duplicate tasks |
| Warm latency | Fresh wall-clock duration for an immediate exact repeat, with cache-hit, same-selection and HTTP counts recorded |
